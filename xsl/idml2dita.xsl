...
version="3.0">
  ...
  <xd:doc
    ...
    scope="stylesheet">
    <xd:desc>
      <xd:p>IDML to DITA generic transformation</xd:p>
      <xd:p>Copyright (c) 2021 DITA For Publishers, Inc.</xd:p>
      <xd:p>Transforms an IDML designmap.xml file into a DITA topic using a style-to-tag mapping. </xd:p>
      <xd:p>This transform is intended to be the base for more specialized transforms that provide
        style-specific overrides. The input to this transform is the document.xml file within an IDML
        package. </xd:p>
      <xd:p>This transform requires that all content to be converted be in InDesign articles.</xd:p>
    </xd:desc>
  </xd:doc>
  ...
  <xsl:variable name="isDita12" as="xs:boolean" select="$ditaVersion ne '1.3'" />
  
  <xsl:variable name="isDita13" as="xs:boolean" select="$ditaVersion = ('1.3')" />
  ...
  <xsl:template
    ...
    <xsl:variable name="stylesDoc" as="document-node()"
      select="document('Resources/Styles.xml', .)"
    />      
